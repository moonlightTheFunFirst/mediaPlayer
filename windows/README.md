...
音声判定は拡張子ではなく音声・映像トラックを使う。ファイル選択にはWAV・MP3・FLAC・M4A等の音声フィルターを追加。エフェクトはQt 6.8のQAudioBufferOutput（FFmpegバックエンド）を使い、追加の配布DLLは不要。音声データを取得できないバックエンドでは静止した「♬」を表示する。ジャンル・感情の自動判定は行わない。
